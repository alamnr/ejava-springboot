...
public class SongsControllerNTest {
    ...
    void update_song() {
        ...
        BDDAssertions.then(updatedSong.getTitle()).isNotEqualTo(originalTitle);
    ...
    void delete_song() {
        ...
        // then
        response.expectStatus().isNoContent();
        wtc.get().uri(SongsController.SONG_PATH,song.getId()).exchange().expectStatus().isNotFound();
    ...
    class Finders {
        ...
        void find_all(String mediatypeValue) throws IOException {
            ...
            // when
            log.info("*********************** example uri - {} ", findByExampleUriBuilder.build().toUri().toString());
            WebTestClient.RequestHeadersSpec<?> request = wtc.post().uri(findByExampleUriBuilder.build().toUri())
            ...
            SongsPageDTO pageDTO = request.exchange().expectStatus().isOk().returnResult(SongsPageDTO.class)
                                                        .getResponseBody().blockFirst();
        ...
        void find_all_paged(String mediaTypeValue) {
            ...
            while (page==null || page.getNumberOfElements()>0) {
                ...
                log.info("*********************** example uri - {} ", uri.toString());
                WebTestClient.RequestHeadersSpec<?> request = wtc.post()
                        .uri(uri)
                        .contentType(mediaType)
                        .body(Mono.just(allSongsProbe), SongDTO.class)
        ...
        void findall_sorted_restTemplate(String mediaTypeValue) throws IOException {
            ...
            List<SongDTO> matchingSongs = songs.values().stream()
                    ...
                    .sorted((s1,s2)->s1.getReleased().compareTo(s2.getReleased()))
                    .toList();
            PageRequest pageableRequest = PageRequest.of(0, 5, Sort.by(Sort.Order.desc("released")));
            PageableDTO pageSpec = PageableDTO.of(pageableRequest);
            SongDTO allSongsProbe = SongDTO.builder().build();
            log.info("probe={}", dtoUtil.marshal(allSongsProbe));
            ...
            //then
            then(response.getStatusCode().is2xxSuccessful()).isTrue();
            SongsPageDTO pageDTO = response.getBody();
            log.info("{}", pageDTO);
            then(pageDTO.getNumberOfElements()).isEqualTo(pageSpec.getPageSize());
            then(pageDTO.getTotalElements()).isEqualTo(songs.size());
            LocalDate previous = null;
            ...
            then(pageDTO.getPageableDTO()).isNotNull();
            then(pageDTO.getPageSize()).isEqualTo(pageableRequest.getPageSize());
            then(pageDTO.getPageNumber()).isEqualTo(pageableRequest.getPageNumber());
            then(pageDTO.getSort()).isEqualTo("released:DESC");
